...
class Network(object):
    ...
    def __init__(self, inputs, is_training,
                 dropout_rate=0.5, seed=None, epsilon=1e-5, reuse=False, fcn=True, regularize=True,dilation=1,
                 **kwargs):
    ...
    def reduce_mean(self,
                    ...
                    keepdims,
                    name=None):
        return tf.reduce_mean(input_tensor,axis,keepdims,name)
    ...
    def tile(self,
             ...
             multiples,
             name=None):
        return tf.tile(input_tensor,multiples,name)
    ...
    def convs_gn(self,
        ...
        conv0 = self.conv(input_tensor, 5, filters/4, strides, name+"dilation", relu=True,
                         dilation_rate=dilation_rate, padding=padding,
                         biased=True, reuse=self.reuse, separable=separable)
        conv1 = self.conv(input_tensor, 5, filters/4, strides, name+"dilation", relu=True,
                         dilation_rate=dilation_rate*2, padding=padding,
                         biased=True, reuse=self.reuse, separable=separable)
        conv2 = self.conv(input_tensor, 5, filters/4, strides, name+"dilation", relu=True,
                         dilation_rate=dilation_rate*4, padding=padding,
                         biased=True, reuse=self.reuse, separable=separable)
        conv3 = self.conv(input_tensor, 5, filters/4, strides, name+"dilation", relu=True,
                         dilation_rate=dilation_rate*8, padding=padding,
                         biased=True, reuse=self.reuse, separable=separable)

        conv=tf.concat([conv0,conv1,conv2,conv3],-1)
        conv=self.conv(conv, kernel_size, filters, strides, name, relu=False,
                         dilation_rate=dilation_rate, padding=padding,
